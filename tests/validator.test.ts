...
describe("extended definition rules", () => {
  const extended = {
    client: "demo-client",
  ...
  it("supports cross-field date comparisons", () => {
    ...
    const invalid = validate(extended, {
      start_date: "2026-02-10",
      end_date: "2026-02-05",
      requires_approval: false,
    });
    expect(errorsFor(invalid, "end_date")).toHaveLength(1);
  });

  it("supports conditional required fields without cascading dependent errors", () => {
    expect(
      errorsFor(
        validate(extended, {
          start_date: "2026-02-01",
    ...
    const invalidDependency = validate(extended, {
      start_date: "not-a-date",
      end_date: "2026-02-05",
      requires_approval: false,
    });
    expect(errorsFor(invalidDependency, "start_date")).toHaveLength(1);
    expect(errorsFor(invalidDependency, "end_date")).toHaveLength(0);
  });
});
